...
var fancyNavInit = function fancyNavInit() {
  ...
  if (fancynavbar) {
    ...
    var fancyOnePagerDropdownItems = document.querySelectorAll(Selector.ONE_PAGER_DROPDOWN_ITEM);
    if (fancyOnePagerDropdownItems.length) {
      fancyOnePagerDropdownItems.forEach(function (navLink) {
        navLink.addEventListener(Events.CLICK, function () {
          animateMenu();
        });
      });
    }
